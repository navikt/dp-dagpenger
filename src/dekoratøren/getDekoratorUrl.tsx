...
export function getDekoratorUrl(params?: DekoratorParams): string {
  ...
  const stringified = {
    ...
    availableLanguages:
      params.availableLanguages && JSON.stringify(params.availableLanguages),
    breadcrumbs: params.breadcrumbs && JSON.stringify(params.breadcrumbs),
  };

  const filtered = Object.entries(stringified).filter(
  ...
  return (
    baseUrl + "?" + filtered.map((param) => `${param[0]}=${param[1]}`).join("&")
  );
}
