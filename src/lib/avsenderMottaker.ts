...
const BRUKER_ER_AVSENDERMOTTAKER = (j: JP) => j.brukerErAvsenderMottaker;

export const sendtFraBrukerTilNAV = (j: JP) =>
  INNKOMMENDE(j) && BRUKER_ER_AVSENDERMOTTAKER(j);
export const sendtFraEksternPartTilNAV = (j: JP) =>
  INNKOMMENDE(j) && !BRUKER_ER_AVSENDERMOTTAKER(j);
export const sendtFraNAVTilBruker = (j: JP) =>
  UTGAAENDE(j) && BRUKER_ER_AVSENDERMOTTAKER(j);
export const sendtFraNAVTilEksternPart = (j: JP) =>
  UTGAAENDE(j) && !BRUKER_ER_AVSENDERMOTTAKER(j);

export const meldingInternt = (j: JP) =>
  NOTAT(j) && !BRUKER_ER_AVSENDERMOTTAKER(j);
